...
# Description: This script will creates a HA Api Connect Passive instance on the DR Data center.
#
...
# get ingress host of the ocp cluster - READ FROM inputs.properties
# PASSIVE_INGRESS_DOMAIN=$(oc get ingresses.config/cluster -o jsonpath={.spec.domain})
    ...
    multiSiteHA:
      ...
      replicationEndpoint:
        annotations:
          cert-manager.io/issuer: ${APIC_INSTANCE_NAME}-ingress-issuer
        hosts:
        - name: mgmt-replication.${PASSIVE_INGRESS_DOMAIN}
          secretName: ${APIC_INSTANCE_NAME}-mgmt-replication-server
      replicationPeerFQDN: mgmt-replication.${ACTIVE_INGRESS_DOMAIN}
      tlsClient:
    ...
    multiSiteHA:
      ...
      replicationEndpoint:
        annotations:
          cert-manager.io/issuer: ${APIC_INSTANCE_NAME}-ingress-issuer
        hosts:
        - name: ptl-replication.${PASSIVE_INGRESS_DOMAIN}
          secretName: ${APIC_INSTANCE_NAME}-ptl-replication-server
      replicationPeerFQDN: ptl-replication.${ACTIVE_INGRESS_DOMAIN}
      tlsClient:
...
oc get mgmt -o jsonpath='{.items[0].status.haStatus}' | jsonlint
